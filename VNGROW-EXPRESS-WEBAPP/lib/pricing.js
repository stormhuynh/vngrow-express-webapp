...
function findCountryRow(country) {
  ...
  return DB.COUNTRY_ZONE_MAPPING.find((r) => {
    ...
    if (al) {
      return al.split(/[;,|]/).map((x) => x.trim()).includes(country.trim().toLowerCase());
    }
    return false;
  });
}
...
export function computeQuote(input) {
  ...
  if (!zone) return { ok: false, reason: `Tuyến ${destination_country} qua ${service_code} chưa phục vụ`, need_human: true };
  ...
  // surcharges (auto = đo được; manual = optional)
  ...
  return {
    ok: true, service_code, destination_country, zone,
    gw, vw, cw, base, addons, optional, vat, total,
    currency: "VND",
  };
}
